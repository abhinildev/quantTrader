...
"""
Strategies:
- Market making for ASH
- Mean reversion for INT
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            if product == "ASH_COATED_OSMIUM":
                ...
                buy_price = int(min(best_bid + 1 - skew, best_ask - 1))
                sell_price = int(max(best_ask - 1 - skew, best_bid + 1))
            ...
            ### INT
                ...
                vol = 0.6 * prev_vol + 0.4* abs(mid_price - prev_price)

                z = (mid_price - fair_price) / max(vol, 1)

                base_size = 6
                size = min(base_size + int(abs(z)), 10)

               
                buy_price = int(min(best_bid + 1, best_ask - 1))
                sell_price = int(max(best_ask - 1, best_bid + 1))
                ...
                if z < -0.7:
                    ...
                    if position < 10:
                        orders.append(Order(product, buy_price, 4))
                    if position > -10:
                        orders.append(Order(product, sell_price, -4))

                data[product + "_vol"] = vol
                data[product] = mid_price
